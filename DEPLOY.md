# DEPLOY.md — ai-daily-web 최초 배포 런북
...
| 사람이 보는 주소 | `https://daily.onebitecoder.com/ai` (루트 `/`는 `/ai/`로 301) |
| 컨테이너 노출 포트 | `127.0.0.1:8021` (btc-daily-web이 8020) |
| compose 프로젝트명 | `ai-daily-web` (디렉토리명 — btc와 갈라져 볼륨·네트워크가 안 겹친다) |
...
# 8021 이 비어 있는지. 뭔가 물려 있으면 멈추고 사람에게 물어라
sudo lsof -iTCP:8021 -sTCP:LISTEN || echo "8021 비어 있음 (정상)"
...
```bash
grep -n "base: '/ai/'" frontend/vite.config.ts
grep -n 'basename="/ai"' frontend/src/App.tsx
...
| `WEB_PORT` | `8021` | `.env.example` 기본값 그대로 |
...
> `ai-daily-web/backend/.env`에서 그 줄을 가져와 여기 붙여넣어야 한다.
>
...
```bash
sudo lsof -iTCP:8021 -sTCP:LISTEN    # 127.0.0.1:8021 이어야 한다. *:8021 이면 잘못됐다
```
...
역할이 다르다 — 이건 TLS 종단 + `127.0.0.1:8021` 프록시만 한다.
...
```bash
cd ~/meeting_room/lab/ai-daily-web/backend && source .venv/bin/activate
...
**확인**: `https://daily.onebitecoder.com/ai/d/2026-08-29`를 브라우저로 연다.
...
```bash
cp deploy/com.nsw.ai-daily.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.nsw.ai-daily.plist
...
- **`web` 서비스를 `0.0.0.0`에 바인딩하지 마라.** 공인 IP:8021로 TLS와
